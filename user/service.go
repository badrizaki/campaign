...
type Service interface {
	...
	Login(input LoginUserInput) (User, error, int)
...
func (s *service) Login(input LoginUserInput) (User, error, int) {
	email := input.Email
	...
	if user.ID == 0 {
		return user, errors.New("User not found"), http.StatusUnauthorized
	}
